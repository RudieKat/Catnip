...
public class FastCRC16 {
    ...
    public static short calculate(final ByteBuffer bytes) {
        int crc = 0;
        for (int i = 0; i < bytes.position();i++) {
            crc = (crc << 8) ^ table[(crc >>> 8 ^ bytes.get(i)) & 0xff];
        }
        return (short)(crc&0xffff);
    }
    public static boolean check(final ByteBuffer bytes) {
        short val = bytes.getShort();
        bytes.position(bytes.position()-2);
        return val == calculate(bytes);
    }
